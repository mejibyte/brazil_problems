...
unsigned long long choose[70][70];
int need[70];
bool been[70][70][2];
unsigned long long memo[70][70][2];

// contar cuantos numeros hay <= a digits que tengan exactamente k bits en 1
unsigned long long f( const vector< int > &digits, int i, int k, bool smaller ) {
    if (i == digits.size()){
        return (k == 0);
    }
    
    if (been[i][k][smaller]) return memo[i][k][smaller];
    
    unsigned long long &ans = memo[i][k][smaller];
    ans = 0;
    // place 0
    ans += f(digits, i + 1, k, smaller || (digits[i] == 1));
    // place 1
    if (smaller or digits[i] == 1) ans += f(digits, i + 1, k - 1, smaller);
    been[i][k][smaller] = true;
    return ans;
}


unsigned long long count(unsigned long long limit, int target) {
    if (limit <= 64) {
    ...
    for (int i = 1; i <= 64; ++i) {
        if (need[i] == target - 1) {
            ...
            unsigned long long extra = f( digits, 0, i, false );
            if (target == 1) extra--;
            assert( extra >= 0 );
            ans += extra;
        }
...
void check(int lo, int hi, int x) {
    unsigned long long ans = count(hi, x) - count(lo - 1, x);
...
//    For(i, 0, 10000){
//        int lo = rand() % 1000000 + 1;
//        int hi = rand() % 1000000 + 1;
//        if (hi < lo) swap(lo, hi);
//        int x = rand() % 11;
//        check(lo, hi, x);
//        printf(".");
