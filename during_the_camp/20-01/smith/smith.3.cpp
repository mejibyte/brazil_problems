...
int main(){
    ...
    for (int g = 0; g < 2; ++g) {
        scanf("%d %d", &N[g], &M[g]);
        assert(1 <= N[g] and N[g] <= 100000);
        assert(1 <= M[g] and M[g] <= 100000);
    ...
    while (T--) {
        ...
        if (g0 == oo) { assert(g1 != oo);
            bool ans = hasGrundy(u0, G[0], grundy[0], g1);
            puts(ans ? "first" : "draw");
        } else { assert(g0 != oo and g1 == oo);
            bool ans = hasGrundy(u1, G[1], grundy[1], g0);
            puts(ans ? "first" : "draw");
        }
    }
    return 0;
