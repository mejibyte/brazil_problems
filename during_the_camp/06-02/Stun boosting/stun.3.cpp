...
bool round(int r) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        while (timeLeft >= 7) {
            ...
            timeLeft -= 7;
            if (score[who] >= X) {
                winners.push_back( make_pair(minA + timeElapsed, who ) );
            }
        }        
    }
    for (int i = 0; i < 3; ++i) {
        int who = teamB[i];
        int timeLeft = GAME_TIME - minB;
        int timeElapsed = 0;
        while (timeLeft >= 7) {
            score[who] += STUN;
            timeElapsed += 7;
            timeLeft -= 7;
            if (score[who] >= X) {
                winners.push_back( make_pair(minB + timeElapsed, who ) );
            }
        }        
    }
