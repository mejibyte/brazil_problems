...
char dfs0(char depth, int remain, char alpha, char beta) {
    ...
    for (int take = remain; take; take = take & (take - 1)) {
        char t = dfs1(depth - 1, remain ^ (take & -take), alpha, beta);
        if (t > alpha) b = alpha = t;
        if (alpha >= beta) break;
    }
    return alpha;
}
...
char dfs1(char depth, int remain, char alpha, char beta) {
    ...
    for (int take = remain; take; take = take & (take - 1)) {
        char t = dfs0(depth - 1, remain ^ (take & -take), alpha, beta);
        if (t < beta) b = beta = t;
        if (alpha >= beta) break;
    }
    return beta;
...
int main(){
    ...
    int ans = dfs0(L, (1 << 26) - 1, -wordCount[1], +wordCount[0]);
    printf("%d\n", ans);
    return 0;
}