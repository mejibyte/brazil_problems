...
int dfs1(int depth, int remain, int alpha, int beta);

int dfs0(int depth, int remain, int alpha, int beta) {
    if (depth == L) return score(remain);
    if (!canFormWord(remain)) return 0;
    for (int take = 0; take < 26; ++take) if (remain & (1 << take)) {
        alpha = max(alpha, dfs1(depth + 1, remain - (1 << take), alpha, beta));
        if (alpha >= beta) break;
    }
    return alpha;
}
...
int dfs1(int depth, int remain, int alpha, int beta) {
    ...
    if (!canFormWord(remain)) return 0;
    for (int take = 0; take < 26; ++take) if (remain & (1 << take)) {
        beta = min(beta, dfs0(depth + 1, remain - (1 << take), alpha, beta));
        if (alpha >= beta) break;
    }
    return beta;
}
